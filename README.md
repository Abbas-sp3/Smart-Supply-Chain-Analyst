...
SSCA monitors every layer of global import supply chains simultaneously. When a conflict breaks out near the Strait of Hormuz, a cyclone hits a major port, or China restricts rare earth exports, the platform instantly traces the ripple effect across global trade flows — and recommends what to do about it.
...
│   PROCUREMENT    │   RESERVES        │   SIMULATOR (MCTS)│   ALERTS         │
│                  │                   │                   │                  │
│  Tracks supplier │  Monitors         │  Monte Carlo Tree │  Live event feed │
...
⑨ OPERATIONAL RECOMMENDATION
      │
      ▼
⑩ GLOBAL IMPORT IMPACT STATEMENT
```
...
### Step 4 — Run the Platform (Dual Stack)

Start both the Next.js frontend and the Python Monte Carlo Risk Server simultaneously:
...
- The Python Monte Carlo MCTS Risk Server at `http://localhost:8787`

> *Note: The platform is designed with zero-downtime gracefulness. If the Python server is not running, the dashboard falls back to static severity models without crashing.*
